...
if (isset($_POST["ordersalesid"])) {
     ...
     $queryordersales = "SELECT * FROM `orderrent`,`product` WHERE `orderrent`.`orderrent_productid`= `product`.`Product_ID` AND`orderrent_ID`='" . $_POST["ordersalesid"] . "'";
     $resultordersales = mysqli_query($conn, $queryordersales);
     ...
     while ($rowodersales = mysqli_fetch_array($resultordersales)) {
          ...
          <div class="col-12 col-sm-12 text-sm-center col-md-4 text-md-right row">
               ...
               <h6>  <strong>' . $rowodersales["orderrent_unit"] . ' <span class="text-muted"> เครื่อง</span></strong></h6>
               </div>
               <div class="col-3 col-sm-3 col-md-6 text-md-right" style="padding-top: 5px">
                    <h6>รวม  <strong>' . $rowodersales["orderrent_ price"] . ' <span class="text-muted">
                      บาท</span></strong></h6>
               </div>
...
';
     ...
     $mama = 0;
     $selectordersales = "SELECT * FROM `orders`,`user`,`delivery` WHERE `user`.`User_ID`=`orders`.`User_ID`
