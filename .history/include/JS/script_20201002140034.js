...
$(".profile-input").on("change", function () {
  ...
  if (this.files[0]) {
    ...
    reader.onload = function (e) {
      //console.log(reader);
      $("#imgprofile").attr("src", e.target.result).width(240).height(200);
    };
...
$(".product-file").on("change", function () {
  ...
  if (this.files[0]) {
    ...
    reader.onload = function (e) {
      //console.log(reader);
      $("#imgproduct").attr("src", e.target.result).width(800).height(600);
    };
    reader.readAsDataURL(this.files[0]);
...
$(".updateimgproduct-input").on("change", function () {
  ...
  if (this.files[0]) {
    ...
    reader.onload = function (e) {
      //console.log(reader);
      $("#imgprofile").attr("src", e.target.result).width(800).height(600);
    };
    reader.readAsDataURL(this.files[0]);
  ...
  $(document).on("click", ".edit_dataproduct", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#rentday2").val(data.Product_rentday);
...
$(document).ready(function () {
  ...
  $(document).on("click", ".edit_data", function () {
    // เรียกฟอมมาแก้ไข
    var typeproduct_id = $(this).attr("id");
    $.ajax({
      url: "phpsql/fetch.php",
      method: "POST",
      data: {
        typeproduct_id: typeproduct_id, //  ชื่อโพส
      },
      dataType: "json",
      success: function (data) {
        $("#name").val(data.Type_Name);
        $("#address").val(data.Type_ID);
        //$('#gender').val(data.gender);
        //$('#designation').val(data.designation);
        //$('#age').val(data.age);
        $("#typeproduct_id").val(data.Type_ID);
        $("#insert").val("อัพเดท"); // แก้ตรงนี้
